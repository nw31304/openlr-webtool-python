...
class Line(AbstractLine):
    ...
    def __init__(self, map_reader: WebToolMapReaderSQLite, line_id: int, meta: str, fow: FOW, frc: FRC, length: float,
...
class Node(AbstractNode):
    ...
    def __init__(self, map_reader: WebToolMapReaderSQLite, node_id: int, lon: float, lat: float):
        self.lon = lon
        self.lat = lat
        self.map_reader = map_reader
        self.id = node_id
...
# @MapReader.register
class WebToolMapReaderSQLite(MapReader):
    """
    ...
    def __init__(self, db_filename: str, geo_tool: GeoTool, mod_spatialite: str = "mod_spatialite", lines_table: str = "line",
        ...
        self.node_cache = {}
        self.line_cache = {}
        self.line_query_select = "select id,meta,fow,flowdir,frc,len,from_int,to_int,st_asbinary(geom) as geom"
        self.rev_line_query_select = "select -id,meta,fow,flowdir,frc,len,to_int as from_int,from_int as to_int,st_asbinary(st_reverse(geom)) as geom"
        ...
        self.get_lines_query = self.line_query + " where flowdir in (1,3) union " + self.rev_line_query + " where flowdir in (1,2)"
        ...
        {self.line_query_select} from candidates c where c.flowdir in (1,3)
        union 
        {self.rev_line_query_select} from candidates c where c.flowdir in (1,2)
        """
        self.outgoing_lines_query = self.line_query + " where from_int = ? and flowdir in (1,3) union " + self.rev_line_query + " where to_int = ? and flowdir in (1,2)"
        self.incoming_lines_query = self.line_query + " where to_int = ? and flowdir in (1,3) union " + self.rev_line_query + " where from_int = ? and flowdir in (1,2)"
    ...
    def find_nodes_close_to(self, coord: Coordinates, dist: float) -> Iterable[Node]:
        ...
        with closing(self.connection.cursor()) as cursor:
            # start_time = time()
            cursor.execute(self.find_nodes_close_to_query, (lons[0], lats[0], lons[1], lats[1]))
            # end_time = time();
